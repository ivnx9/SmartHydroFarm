...
void handleRelay() {
  ...
  // you can extend to nutrient/pH relays if needed
...
void handleDataJson() {
  ...
  auto addFloat = [&](const char* key, float value, bool last) {
    json += "\""; json += key; json += "\":";
    if (isfinite(value)) {
      json += String(value, 2);
    } else {
  ...
  // sensor values
  addFloat("ph",    phValue,    false);
  addFloat("water", waterTemp,  false);
  addFloat("tds",   tds,        false);
  addFloat("air",   airTemp,    false);
  addFloat("hum",   humidity,   false);
...
void parseUNOData(String input) {
  ...
  if (input.startsWith("ph:")) {
    ...
    phValue = input.substring(p1 + 3, p2).toFloat();

    int t1 = input.indexOf("t:", p2);
    int t2 = input.indexOf(",", t1);
    waterTemp = input.substring(t1 + 2, t2).toFloat();

    int s1 = input.indexOf("ppm:", t2);
    int s2 = input.indexOf(",", s1);
    tds = input.substring(s1 + 4, s2).toFloat();

    int a1 = input.indexOf("airT:", s2);
    int a2 = input.indexOf(",", a1);
    airTemp = input.substring(a1 + 5, a2).toFloat();

    int h1 = input.indexOf("hum:", a2);
    int h2 = input.indexOf(",", h1);
    humidity = input.substring(h1 + 4, h2).toFloat();

    int hh1 = input.indexOf("hour:", h2);
    int hh2 = input.indexOf(",", hh1);
    hour = input.substring(hh1 + 5, hh2).toInt();

    int mm1 = input.indexOf("minute:", hh2);
    int mm2 = input.indexOf(",", mm1);
    minute = input.substring(mm1 + 7, mm2).toInt();

    int ss1 = input.indexOf("second:", mm2);
    second = input.substring(ss1 + 7).toInt();

   Serial.printf("UNO -> PH: %.2f, Temp: %.2f, PPM: %.2f, airTemp: %.2f, hum: %.2f, hour: %02d, minute: %02d, second: %02d\n", phValue, waterTemp, tds, airTemp, humidity, hour, minute, second);
    addLog(String("UNO -> pH: ") + String(phValue,2) + ", ppm: " + String(tds,0) + ", airT: " + String(airTemp,1) + ", hum: " + String(humidity,1));
  }
}
...
void parseUNOData(String input) {
  ...
  // --- 1) Normal sensor line: "ph:...,t:...,ppm:...,airT:...,hum:...,hour:...,minute:...,second:..." ---
  if (input.startsWith("ph:")) {
    ...
    Serial.printf(
      "UNO -> PH: %.2f, Temp: %.2f, PPM: %.2f, airTemp: %.2f, hum: %.2f, hour: %02d, minute: %02d, second: %02d\n",
      phValue, waterTemp, tds, airTemp, humidity, hour, minute, second
    );
...
void intelligentAdjustments() {
  ...
  static bool warnedPlantOnce = false;

  // 1) Learn from previous action if we've waited long enough
  maybeLearnFromLastAction();

  // 2) Respect cooldown between new doses
  if (millis() - lastDoseTime < COOLDOWN_MS) return;

  // 3) Basic sensor sanity
  if (!isfinite(phValue) || phValue < 0.1f || phValue > 14.0f) {
    Serial.println("[SKIP] Bad pH reading.");
    return;
  }
  // TDS can be NaN occasionally; we’ll gate TDS logic later.

  // 4) Validate/repair plant config so we never get target=NaN
  bool plantOK = true;
  if (!isfinite(currentPlant.ph_min)  || !isfinite(currentPlant.ph_max) ||
      currentPlant.ph_min < 4.0f      || currentPlant.ph_max > 8.5f     ||
      currentPlant.ph_min >= currentPlant.ph_max) {
    plantOK = false;
  }
  float target = currentPlant.ph_target;
  if (!isfinite(target)) plantOK = false;

  if (!plantOK) {
    if (!warnedPlantOnce) {
      Serial.println("[WARN] Plant config invalid/missing. Repairing to safe defaults.");
      warnedPlantOnce = true;
    }
    // Safe repair
    currentPlant.ph_min    = 5.5f;
    currentPlant.ph_max    = 6.5f;
    target                 = 6.0f;
    currentPlant.ppm_min   = (currentPlant.ppm_min  > 0) ? currentPlant.ppm_min  : 560;
    currentPlant.ppm_max   = (currentPlant.ppm_max  > currentPlant.ppm_min) ? currentPlant.ppm_max : 840;
  } else {
    warnedPlantOnce = false;
    // If target isn’t inside [min,max], nudge it into range
    if (!isfinite(target) || target < currentPlant.ph_min || target > currentPlant.ph_max) {
      target = 0.5f * (currentPlant.ph_min + currentPlant.ph_max);
    }
  }

// 5) TDS first: if below min, add nutrients toward midpoint in safe steps
if (isfinite(tds) && currentPlant.ppm_min > 0 && currentPlant.ppm_max > currentPlant.ppm_min) {
  const float target_ppm = currentPlant.ppm_min +
                           TARGET_PPM_FRACTION * (currentPlant.ppm_max - currentPlant.ppm_min);
  if (tds < target_ppm) {
    float need_ppm = target_ppm - tds;
    if (need_ppm >= TDS_MIN_NEED_PPM) {
      // take only a fraction, and cap the per-shot step
      float step_ppm = need_ppm * TDS_STEP_FRACTION;
      if (step_ppm > TDS_MAX_STEP_PPM) step_ppm = TDS_MAX_STEP_PPM;

      // predict ms from model (NO exploration for TDS — keeps it tame)
      unsigned long ms = (unsigned long)(model.k_tds * step_ppm);
      ms = clampMs(ms);

      Serial.printf("[TDS] cur=%.0f target=%.0f need=%.0f step=%.0f → dose %lums\n",
                    (double)tds, (double)target_ppm, (double)need_ppm, (double)step_ppm, ms);

      applyDosePair(RELAY_NUTRIENT_A, RELAY_NUTRIENT_B, ms);
      digitalWrite(RELAY_MIXER, LOW); delay(MIX_AFTER_TDS_MS); digitalWrite(RELAY_MIXER, HIGH);

      lastAction   = { ACT_TDS, millis(), tds, (float)ms };
      lastDoseTime = millis();
      return; // one actuator per loop
    }
  }
}
  ...
  // 6) pH control (respect plant bounds + tolerance)
  const float tol = PH_TOL;

  // Tighten target toward bounds if we drift outside
  if (phValue < currentPlant.ph_min - 0.05f) target = currentPlant.ph_min;
  if (phValue > currentPlant.ph_max + 0.05f) target = currentPlant.ph_max;

  float diff = target - phValue; // + => need pH UP ; - => need pH DOWN
  if (!isfinite(diff)) {
    Serial.println("[SKIP] diff is NaN (bad target).");
    return;
  }
  if (fabsf(diff) <= tol) {
    // Inside deadband
  ...
  // Absolute rails — never push farther in the dangerous direction
  if (phValue < PH_SAFETY_MIN && diff < 0) { Serial.println("[SKIP] Low pH, refusing to dose DOWN."); return; }
  if (phValue > PH_SAFETY_MAX && diff > 0) { Serial.println("[SKIP] High pH, refusing to dose UP.");  return; }

  // Limit per-shot size to avoid big jumps
  if (diff >  MAX_PH_STEP_PER_SHOT) diff =  MAX_PH_STEP_PER_SHOT;
  if (diff < -MAX_PH_STEP_PER_SHOT) diff = -MAX_PH_STEP_PER_SHOT;

  if (diff > 0) {
    // Need to increase pH -> PH_UP
    unsigned long ms = (unsigned long)(model.k_ph_up * diff);
    ms = clampMs(ms);
    ms = explore(ms, model.updates_up);

    Serial.printf("[pH↑] cur=%.2f target=%.2f diff=%.2f → dose %lums\n",
                  (double)phValue, (double)target, (double)diff, ms);

    applyDoseSingle(RELAY_PH_UP, ms);
    digitalWrite(RELAY_MIXER, LOW); delay(MIX_AFTER_PH_MS); digitalWrite(RELAY_MIXER, HIGH);

    lastAction   = { ACT_PH_UP, millis(), phValue, (float)ms };
    lastDoseTime = millis();
  } else {
    // Need to decrease pH -> PH_DOWN
    float mag = -diff;
    unsigned long ms = (unsigned long)(model.k_ph_down * mag);
    ms = clampMs(ms);
    ms = explore(ms, model.updates_down);

    Serial.printf("[pH↓] cur=%.2f target=%.2f diff=%.2f → dose %lums\n",
                  (double)phValue, (double)target, (double)diff, ms);

    applyDoseSingle(RELAY_PH_DOWN, ms);
    digitalWrite(RELAY_MIXER, LOW); delay(MIX_AFTER_PH_MS); digitalWrite(RELAY_MIXER, HIGH);

    lastAction   = { ACT_PH_DOWN, millis(), phValue, (float)ms };
    lastDoseTime = millis();
  }
}
